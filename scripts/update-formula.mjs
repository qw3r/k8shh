...
if (!sha) {
  const shaFile = resolve(root, 'dist', `gap-secrets-${version}.tar.gz.sha256`);
...
const url = `https://github.com/${repo}/releases/download/v${version}/gap-secrets-${version}.tar.gz`;

const formula = `class GapSecrets < Formula
...
writeFileSync(resolve(dir, 'gap-secrets.rb'), formula);
console.log(`Wrote Formula/gap-secrets.rb (version=${version}, repo=${repo})`);
